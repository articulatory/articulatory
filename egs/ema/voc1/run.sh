...
if [ "${stage}" -le 1 ] && [ "${stop_stage}" -ge 1 ]; then
    ...
    if [ -z "${pretrain}" ]; then
        ...
        ${train_cmd} "${dumpdir}/${train_set}/compute_statistics.log" \
            articulatory-compute-statistics \
                --config "${conf}" \
...
if [ "${stage}" -le 2 ] && [ "${stop_stage}" -ge 2 ]; then
    ...
    if [ "${n_gpus}" -gt 1 ]; then
        train="python -m articulatory.distributed.launch --nproc_per_node ${n_gpus} -c articulatory-train"
    else
        train="articulatory-train"
    fi
